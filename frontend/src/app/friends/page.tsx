  ...
  const sendRequestMutation = useMutation({
    ...
    onError: (err: { response?: { status?: number; data?: { message?: string } } }) => {
      if (err?.response?.status === 409) {
        setSendError('이미 친구 요청을 보냈거나 이미 친구입니다.')
      } else if (err?.response?.status === 404) {
        setSendError('해당 사용자를 찾을 수 없습니다.')
      } else {
        setSendError('친구 요청에 실패했습니다. 다시 시도해주세요.')
      }
    },
  })
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {showSearch && (
          <div ref={searchRef} className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 shadow-sm">
            <p className="text-sm font-semibold text-gray-700 mb-3">사용자 검색</p>
                ...
                {searchResults.map((user) => {
                  ...
                  return (
                    <div key={user.id} className="flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      {isFriend ? (
                        <span className="text-xs text-green-600 font-medium">이미 친구</span>
                      ) : (
                        <button
        ...
        {pending.length > 0 && (
          <section className="mb-4">
            <h2 className="text-sm font-semibold text-gray-500 mb-2">받은 친구 요청 ({pending.length})</h2>
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              {pending.map((req, idx) => (
                ...
                >
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{req.senderUsername ?? '알 수 없음'}</p>
        ...
        <section>
          ...
          {friendsLoading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 py-12 text-center">
              ...
              <p className="text-sm font-semibold text-gray-700">아직 친구가 없어요</p>
              <p className="text-xs text-gray-400 mt-1">위에서 사용자를 검색해 친구를 추가해보세요!</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
